...
fn calc_happiness_score(arrangement: &Vec<&String>, scores: &HashMap<String, isize>) -> isize {
    ...
    for x in 0..arrangement.len() {
        ...
        score += s;
        let neighbour2 = if x == arrangement.len() - 1 {
...
fn main() {
    ...
    for actor in actors.iter() {
        println!("{:#?}", actor);
        scores.insert(format!("{}_{}", host, actor), 0);
        scores.insert(format!("{}_{}", actor, host), 0);
    }
    actors.insert(host);
    println!("{:#?}", actors);
    println!("{:#?}", scores);
    ...
    println!("max happiness score: {:#?}", happiness_scores.iter().max().unwrap()); // 601
}